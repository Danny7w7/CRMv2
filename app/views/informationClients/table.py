...
def clientObamacarePass(request):
    ...
    if request.user.is_superuser:
        ...
        obamaCare = ObamaCare.objects.select_related('agent','client').exclude(tipe_sale = 'R').filter(created_at__lte=fechaLimite).annotate(
        ...
        obamaCare = ObamaCare.objects.visible_for_user(request.user).select_related('agent','client').exclude(tipe_sale = 'R').filter(created_at__lte=fechaLimite).only(
        ...
        obamaCare = ObamaCare.objects.select_related('agent', 'client').exclude(tipe_sale = 'R').filter(created_at__lte=fechaLimite).only(
            'agent_usa', 'agent__first_name', 'agent__last_name', 'client__first_name', 'client__last_name',
                'client__phone_number', 'status', 'status_color', 'profiling','created_at', 'is_active').annotate(
                    truncated_agent_usa=Substr('agent_usa', 1, 8),
